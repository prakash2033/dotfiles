...
setopt PROMPT_SUBST
export PS1=$'%F{blue}┌──%F{darkgray}[%T]%F{blue}[%(5~|%-1~/…/%3~|%4~)${vcs_info_msg_0_}%F{blue}]\n%F{blue}└─%(?.%F{green}.%F{red})🗡 $%f '
...
alias dot='cd ~/Public/gitrepo/dotfiles && vim +Files' 
...
[[ -z "$TMUX" && -z "$NOTMUX" ]] && tmux new-session -A -s 0
