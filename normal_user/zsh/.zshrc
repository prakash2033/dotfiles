...
setopt PROMPT_SUBST
export PS1='%F{040}%n%f %F{031}${PWD/#$HOME/~}%f${vcs_info_msg_0_} %{$reset_color%}%{$fs_bold[white]%}%(?.%F{040}.%F{red})>%{$reset_color%} '
...
alias ga='git add .'
alias gc='git commit'
alias gch='git checkout $(git branch -a | fzf | cut -c 18-)'
